...
public class MLXLocalLLMService: LLMEngine, InferenceService {
    ...
    private static func takeLiveComponentTestLaunchToken() -> Bool {
        guard ProcessInfo.processInfo.environment["GEMMA4RUNTIME_RUN_LIVE_COMPONENT_TEST"] == "1" else {
            return false
        }
    ...
    public func warmup() async throws {
        ...
        // Gemma4Runtime's debug-only live component test depends on the full live audio
        // stack. Gemma4App vendors the inference runtime separately, so keep the
        // opt-in flag consumed but do not launch the test harness here.
        _ = Self.takeLiveComponentTestLaunchToken()
        #endif
    }
